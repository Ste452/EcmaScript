...
var data = {
  ...
  trial2: [
    [2, 3],
    [4, 5],
  ], // The elements of Arrays are Arrays.
};
...
var x = 2,
  y = 3; // These signs = Are attributions and not a testings of equality
...
(false ==
  ...
  (y == 3)(
    // => true: The both comparacions are true.
    x > 3
  )) ||
  y < 3; // => false: None of comparacions are true.
!(x == y); // => true: ! invert boolean value.

// Functions are code blocks in JavaScript parameterized that we can call.

function plus1(x) {
  // Define a function called "plus1", with parameter "x"
...
points.dist = function () {
  // Define method to calculate the distance between points
...
function abs(x) {
  // One function to calculate the absolute value
  if (x >= 0) {
    // An if instrution...
    return x; // Executes this code, if the comparation is true
  } // This is the end of the if clause
  else {
    // Optional clause else executes your code if the comparation being false
    return -x;
...
function factorial(n) {
  ...
  while (n > 1) {
    // Repeat the instrutions that they are in braces {}, while the expresssion in () being true
...
function factorial2(n) {
  ...
  // Another version, using a different tie
...
function Point(x, y) {
  // By convention, construction companies start with capital letters

  this.x = x; // The this keyword is the new object that is being initialized
  this.y = y; // Store the function arguments as a property of the object
} // Any return is not necessary

// Use a constructor function with the "new" keyword to create instances
var p = new Point(1, 1); // The geometric point (1,1)

// Define methods for Point objects by assigning them to the
...
// This is the Point object on which the...
// ...method is called.
